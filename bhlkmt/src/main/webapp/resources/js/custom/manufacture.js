	...
	if (data.length > 0) {
		for (var i = 0; i < data.length; i++) {
			...
			result.append("<tr><td>"
							...
							+ "\"></td>"
							+ "<td>"
							+ temp.manufactureCode
							+ "</td>"
							+ "<td>"
							+ temp.manufactureName
							+ "</td>"
							+ "<td class=\"actions\">"
							+ "<a onclick=\"edit("
							+ temp.manufactureId
...
function edit(id) {
	...
	$('#titleFormInput').html("<strong>Sửa Thông Tin Nhà Sản Xuất</strong>");
	$('#btnSubmit').text("Sửa");
	$('#code').focus();
	...
	var inputData = {}
	inputData["manufactureId"] = id;
	
	$.ajax({
		type : "POST",
		url : "./manufacture/getById",
		contentType: "application/json; charset=UTF-8",
		data : JSON.stringify(inputData),
		dataType : "json",
		success : function(data) {
			$('#id').val(data.manufactureId);
			$('#code').val(data.manufactureCode);
			$('#name').val(data.manufactureName);
		},
		error : function(jqXHR, textStatus, errorThrown) {
			if (jqXHR.status) {
				alert(jqXHR.responseText);
...
$(document).ready(function() {
	...
	$('#btnCreate').click(function(e) {
		$('#sectionFormInput').css('display', 'block');
		$('#titleFormInput').html("<strong>Thêm Nhà Sản Xuất</strong>");
		$('#btnSubmit').text("Lưu");
		$('#id').val(0);
		$('#code').val("");
		$('#name').val("");
	...
	$("#form3").submit(function(e) {
		if (this.checkValidity()) {
			...
			var inputData = {}
			inputData["manufactureCode"] = $('#code').val();
			inputData["manufactureName"] = $('#name').val();
			...
			if (id <= 0) {
				...
				inputData["manufactureId"] = $('#id').val();
				$.ajax({
					type : "POST",
